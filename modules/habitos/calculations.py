...
Otimização de 2026-07-29 (migração pra Turso): contra um banco local, uma
query por hábito por métrica era de graça. Contra um banco remoto, cada
query custa ~0.3s de rede — uma tela com 5 hábitos fazendo streak+%+marcado
por hábito, mais a lista de hábitos repetida em cada função, virava 20+
round-trips (vários segundos) só de esperar rede. Por isso toda função aqui
aceita `habitos`/`registros` pré-buscados (uma ÚNICA query cada, via
`buscar_habitos()`/`buscar_registros_todos()`) e filtra/calcula em Python —
quem chama isoladamente sem passar esses argumentos ainda funciona (busca
na hora), mas a página busca uma vez só e repassa pra tudo.
"""
...
def buscar_habitos(somente_ativos: bool = True) -> pd.DataFrame:
    """Busca única da lista de hábitos — chamar uma vez por render de
    página/relatório e repassar pras funções abaixo."""
    return models.listar_habitos(somente_ativos)


def buscar_registros_todos() -> pd.DataFrame:
    """Busca única de todos os registros de todos os hábitos — chamar uma
    vez por render de página/relatório e repassar pras funções abaixo."""
...
def esta_marcado_hoje(habito_id: int, hoje: date, registros: pd.DataFrame | None) -> bool:
    """Mesma pergunta de `models.esta_marcado`, mas a partir de um DataFrame
    já buscado em vez de uma query nova."""
    if registros is None:
...
def percentual_periodo(
    ...
    """% de dias cumpridos dentro do intervalo [data_inicio, data_fim]
    (ambos inclusive) — versão de `percentual_cumprido` por período
    arbitrário, em vez de 'últimos N dias a partir de hoje'."""
...
def progresso_por_habito_periodo(
    ...
    """% cumprido de cada hábito ativo dentro do período — versão de
    `progresso_por_habito` por intervalo arbitrário."""
    habitos = _habitos_ou_buscar(habitos)
    if habitos.empty:
        return pd.DataFrame(columns=["nome", "percentual"])
...
def evolucao_semanal(
    ...
    últimas N semanas — usada só no relatório em PDF (a tela usa a versão
    mensal, `cumprimento_por_mes`)."""
    hoje = hoje or date.today()
    habitos = _habitos_ou_buscar(habitos)
    if habitos.empty:
...
def heatmap_diario(
    ...
    """% de hábitos ativos cumpridos em cada um dos últimos `dias` dias —
    base pro heatmap estilo calendário de contribuições (GitHub)."""
    hoje = hoje or date.today()
    inicio = hoje - timedelta(days=dias - 1)
...
def cumprimento_por_mes(
    ...
    """% médio de cumprimento (entre hábitos ativos) por mês, dentro do
    período — granularidade mensal em vez de semanal pra aguentar períodos
    longos (ex.: 'todos os tempos') sem virar um gráfico com dezenas de
    barras, mesmo padrão usado em Música/Livros/Humor."""
