...
checklist e resumo geral. Port de modules/projetos/calculations.py — sem
acesso direto a SQL, usa repo.py."""
...
def resumo_geral() -> dict:
    ...
    ativos = 0 if projetos.empty else int((projetos["status"] == "Ativo").sum())

    total_investido = 0.0
    if not projetos.empty:
        total_investido = sum(valor_acumulado(int(pid)) for pid in projetos["id"])

    return {"projetos_ativos": ativos, "total_investido": round(total_investido, 2)}
