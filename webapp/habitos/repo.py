"""Camada de acesso a dados (CRUD). Port de modules/habitos/models.py, sem
@st.cache_data (ver webapp/musica/repo.py pro raciocínio completo)."""
...
def atualizar_habito(habito_id: int, nome: str, ativo: bool) -> None:
    conn = get_connection()
    conn.execute("UPDATE habitos SET nome = ?, ativo = ? WHERE id = ?", (nome, int(ativo), habito_id))
    conn.commit()
...
def esta_marcado(habito_id: int, data_iso: str) -> bool:
    conn = get_connection()
    row = conn.execute(
        "SELECT 1 FROM habito_registros WHERE habito_id = ? AND data = ?", (habito_id, data_iso),
    ).fetchone()
...
def marcar_dia(habito_id: int, data_iso: str) -> None:
    conn = get_connection()
    conn.execute("INSERT OR IGNORE INTO habito_registros (habito_id, data) VALUES (?, ?)", (habito_id, data_iso))
    conn.commit()


def desmarcar_dia(habito_id: int, data_iso: str) -> None:
    conn = get_connection()
    conn.execute("DELETE FROM habito_registros WHERE habito_id = ? AND data = ?", (habito_id, data_iso))
    conn.commit()
...
def intervalo_datas_registros() -> tuple[str, str] | None:
    conn = get_connection()
    row = conn.execute("SELECT MIN(data) AS minima, MAX(data) AS maxima FROM habito_registros").fetchone()
